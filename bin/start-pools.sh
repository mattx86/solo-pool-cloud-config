...
if [ "${ENABLE_WEBUI}" = "true" ]; then
    if [ -x "${WEBUI_DIR}/bin/solopool-webui" ]; then
        sudo systemctl start solopool-webui && echo "  Started solopool-webui"
    else
        echo "  [SKIP] WebUI binary not found at ${WEBUI_DIR}/bin/solopool-webui"
    fi
fi
...
if [ "${NEED_PAYMENTS}" = "true" ]; then
    if [ -x "${PAYMENTS_DIR}/bin/solopool-payments" ]; then
        sudo systemctl start solopool-payments && echo "  Started solopool-payments"
    else
        echo "  [SKIP] Payment processor binary not found at ${PAYMENTS_DIR}/bin/solopool-payments"
    fi
fi
