...
#
# Creates compressed backups of /opt/solo-pool (excluding blockchain data
...
# Format: solo-pool-backup-YYYYMMDD_HHMMSS_TZ.tar.gz
TIMESTAMP=$(date '+%Y%m%d_%H%M%S')
TZ_OFFSET=$(date '+%z')
BACKUP_FILENAME="solo-pool-backup-${TIMESTAMP}_${TZ_OFFSET}.tar.gz"
...
done < <(find "${BACKUP_DIR}" -name "solo-pool-backup-*.tar.gz" -mtime +${BACKUP_RETENTION_DAYS} 2>/dev/null)
...
BACKUP_COUNT=$(find "${BACKUP_DIR}" -name "solo-pool-backup-*.tar.gz" 2>/dev/null | wc -l)
