...
# =============================================================================
# Determine effective RPC ports based on network mode
if [ "${NETWORK_MODE}" = "testnet" ]; then
    EFFECTIVE_BTC_RPC_PORT="48332"
    EFFECTIVE_BCH_RPC_PORT="48334"
    EFFECTIVE_DGB_RPC_PORT="14023"
    EFFECTIVE_XMR_RPC_PORT="38081"
    EFFECTIVE_ALEO_NETWORK="testnet"
    log "  Network mode: TESTNET (using testnet ports)"
else
    EFFECTIVE_BTC_RPC_PORT="${BITCOIN_RPC_PORT:-8332}"
    EFFECTIVE_BCH_RPC_PORT="${BCH_RPC_PORT:-8334}"
    EFFECTIVE_DGB_RPC_PORT="${DGB_RPC_PORT:-14022}"
    EFFECTIVE_XMR_RPC_PORT="${MONERO_RPC_PORT:-18081}"
    EFFECTIVE_ALEO_NETWORK="mainnet"
    log "  Network mode: MAINNET"
fi

# Read RPC credentials from stored files (generated by install scripts)
# All coins now store credentials in rpc.user and rpc.password files
read_rpc_credential() {
    local cred_file="$1"
    if [ -f "${cred_file}" ]; then
...
EFFECTIVE_BTC_RPC_USER=$(read_rpc_credential "${BITCOIN_DIR}/config/rpc.user")
EFFECTIVE_BTC_RPC_PASSWORD=$(read_rpc_credential "${BITCOIN_DIR}/config/rpc.password")

# BCH RPC credentials
EFFECTIVE_BCH_RPC_USER=$(read_rpc_credential "${BCHN_DIR}/config/rpc.user")
EFFECTIVE_BCH_RPC_PASSWORD=$(read_rpc_credential "${BCHN_DIR}/config/rpc.password")

# DGB RPC credentials
EFFECTIVE_DGB_RPC_USER=$(read_rpc_credential "${DIGIBYTE_DIR}/config/rpc.user")
EFFECTIVE_DGB_RPC_PASSWORD=$(read_rpc_credential "${DIGIBYTE_DIR}/config/rpc.password")

# Monero RPC credentials
EFFECTIVE_XMR_RPC_USER=$(read_rpc_credential "${MONERO_DIR}/config/rpc.user")
EFFECTIVE_XMR_RPC_PASSWORD=$(read_rpc_credential "${MONERO_DIR}/config/rpc.password")

# ALEO RPC credentials
EFFECTIVE_ALEO_RPC_USER=$(read_rpc_credential "${ALEO_DIR}/config/rpc.user")
EFFECTIVE_ALEO_RPC_PASSWORD=$(read_rpc_credential "${ALEO_DIR}/config/rpc.password")
...
password = "x"
node_rpc_url = "http://127.0.0.1:${EFFECTIVE_BTC_RPC_PORT}"
node_rpc_user = "${EFFECTIVE_BTC_RPC_USER}"
node_rpc_password = "${EFFECTIVE_BTC_RPC_PASSWORD}"
EOF
    log "  Bitcoin pool enabled (RPC port: ${EFFECTIVE_BTC_RPC_PORT})"
fi
...
password = "x"
node_rpc_url = "http://127.0.0.1:${EFFECTIVE_BCH_RPC_PORT}"
node_rpc_user = "${EFFECTIVE_BCH_RPC_USER}"
node_rpc_password = "${EFFECTIVE_BCH_RPC_PASSWORD}"
EOF
    log "  Bitcoin Cash pool enabled (RPC port: ${EFFECTIVE_BCH_RPC_PORT})"
fi
...
password = "x"
node_rpc_url = "http://127.0.0.1:${EFFECTIVE_DGB_RPC_PORT}"
node_rpc_user = "${EFFECTIVE_DGB_RPC_USER}"
node_rpc_password = "${EFFECTIVE_DGB_RPC_PASSWORD}"
EOF
    log "  DigiByte pool enabled (RPC port: ${EFFECTIVE_DGB_RPC_PORT})"
fi
...
password = "x"
node_rpc_url = "http://127.0.0.1:${EFFECTIVE_XMR_RPC_PORT}"
node_rpc_user = "${EFFECTIVE_XMR_RPC_USER}"
...
username_format = "YOUR_XMR_ADDRESS.worker_name"
password = "x"
xmr_node_rpc_url = "http://127.0.0.1:${EFFECTIVE_XMR_RPC_PORT}"
xmr_node_rpc_user = "${EFFECTIVE_XMR_RPC_USER}"
xmr_node_rpc_password = "${EFFECTIVE_XMR_RPC_PASSWORD}"
...
node_rpc_user = "${EFFECTIVE_ALEO_RPC_USER}"
node_rpc_password = "${EFFECTIVE_ALEO_RPC_PASSWORD}"
