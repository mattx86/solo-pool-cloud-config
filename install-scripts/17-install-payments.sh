...
PAYMENTS_BASE_URL="${SCRIPTS_BASE_URL%/install-scripts}/payments"

# Create directory structure
mkdir -p ${PAYMENTS_DIR}/src/{wallets,pools}
mkdir -p ${PAYMENTS_DIR}/logs
mkdir -p ${PAYMENTS_DIR}/db
...
# Move binary to payments directory root
cp target/release/solo-pool-payments ${PAYMENTS_DIR}/

# Strip debug symbols
if command -v strip &> /dev/null; then
    strip ${PAYMENTS_DIR}/solo-pool-payments 2>/dev/null || true
    STRIPPED_SIZE=$(du -h ${PAYMENTS_DIR}/solo-pool-payments | cut -f1)
...
cat > ${PAYMENTS_DIR}/config.toml << EOF
...
[database]
path = "${PAYMENTS_DIR}/db/payments.db"
...
if [ "${ENABLE_MONERO_POOL}" = "true" ] && [ "${MONERO_TARI_MODE}" != "tari_only" ]; then
    ...
    XMR_POOL_WALLET_ADDRESS_FILE="${MONERO_DIR}/wallet/pool-wallet.address"
    if [ -f "${XMR_POOL_WALLET_ADDRESS_FILE}" ]; then
        XMR_POOL_WALLET_ADDRESS=$(cat "${XMR_POOL_WALLET_ADDRESS_FILE}")
        log "  Using generated XMR pool wallet: ${XMR_POOL_WALLET_ADDRESS:0:20}..."
    else
        XMR_POOL_WALLET_ADDRESS="${XMR_WALLET_ADDRESS}"
        log "  WARNING: Generated XMR wallet not found, using config value"
    fi
...
if [ "${ENABLE_TARI_POOL}" = "true" ] && [ "${MONERO_TARI_MODE}" != "monero_only" ]; then
    # Read pool wallet address from generated wallet file (if initialized)
    XTM_POOL_WALLET_ADDRESS_FILE="${TARI_DIR}/wallet/pool-wallet.address"
    if [ -f "${XTM_POOL_WALLET_ADDRESS_FILE}" ]; then
        XTM_POOL_WALLET_ADDRESS=$(cat "${XTM_POOL_WALLET_ADDRESS_FILE}")
        log "  Using generated XTM pool wallet: ${XTM_POOL_WALLET_ADDRESS:0:20}..."
    else
        XTM_POOL_WALLET_ADDRESS="${XTM_WALLET_ADDRESS}"
        log "  NOTE: XTM wallet not yet initialized, using config value"
        log "  Run ${TARI_DIR}/wallet/init-wallet.sh after node sync"
    fi
...
[xtm]
enabled = true
# Pool wallet address (from ${TARI_DIR}/wallet/pool-wallet.address)
...
if [ "${ENABLE_ALEO_POOL}" = "true" ]; then
    ...
    ALEO_POOL_WALLET_ADDRESS_FILE="${ALEO_DIR}/wallet/pool-wallet.address"
    ALEO_POOL_PRIVATE_KEY_FILE="${ALEO_DIR}/wallet/pool-wallet.privatekey"

    if [ -f "${ALEO_POOL_WALLET_ADDRESS_FILE}" ]; then
        ...
        ALEO_POOL_WALLET_ADDRESS="${ALEO_WALLET_ADDRESS}"
        log "  WARNING: Generated ALEO wallet not found, using config value"
    fi

    if [ -f "${ALEO_POOL_PRIVATE_KEY_FILE}" ]; then
        ALEO_POOL_PRIVATE_KEY=$(cat "${ALEO_POOL_PRIVATE_KEY_FILE}")
        log "  Using generated ALEO private key"
    else
        ALEO_POOL_PRIVATE_KEY="YOUR_ALEO_PRIVATE_KEY_HERE"
        log "  WARNING: ALEO private key not found"
    fi
...
# Pool private key (from ${ALEO_DIR}/wallet/pool-wallet.privatekey)
...
# =============================================================================
log "8. Creating systemd service..."

cat > /etc/systemd/system/solo-pool-payments.service << EOF
[Unit]
Description=Solo Pool Payment Processor
After=network-online.target
Wants=network-online.target
# Start after pool services
After=pool-xmr-monero-pool.service pool-xmr-xtm-merge-proxy.service pool-aleo.service
...
ExecStart=${PAYMENTS_DIR}/solo-pool-payments ${PAYMENTS_DIR}/config.toml
...
systemctl enable solo-pool-payments >/dev/null 2>&1
...
# 9. SET PERMISSIONS
# =============================================================================
log "9. Setting permissions..."

chown -R ${POOL_USER}:${POOL_USER} ${PAYMENTS_DIR}
chmod 755 ${PAYMENTS_DIR}/solo-pool-payments
chmod 600 ${PAYMENTS_DIR}/config.toml
chmod 755 ${PAYMENTS_DIR}/logs
chmod 755 ${PAYMENTS_DIR}/db

log "  Permissions set"

# =============================================================================
# 10. CREATE SETUP NOTES
# =============================================================================
log "10. Creating setup notes..."
...
DIRECTORIES:
  Binary:   ${PAYMENTS_DIR}/solo-pool-payments
  Config:   ${PAYMENTS_DIR}/config.toml
  Database: ${PAYMENTS_DIR}/db/
...
CONFIGURATION:
  Edit ${PAYMENTS_DIR}/config.toml to:
  - Verify wallet addresses
  - Set ALEO private key (if ALEO enabled)
...
IMPORTANT FOR ALEO:
  You MUST set the pool_private_key in config.toml for ALEO payments to work.
  This key is used to sign outgoing transactions.
EOF
...
log ""
log "  Binary:   ${PAYMENTS_DIR}/solo-pool-payments"
log "  Config:   ${PAYMENTS_DIR}/config.toml"
log "  Database: ${PAYMENTS_DIR}/db/"
...
log ""
if [ "${ENABLE_ALEO_POOL}" = "true" ]; then
    log "  IMPORTANT: Set your ALEO private key in config.toml before starting"
    log ""
fi
