...
for i in $(seq 1 60); do
    if ${DIGIBYTE_DIR}/bin/digibyte-cli -conf=${DIGIBYTE_DIR}/digibyte.conf getblockchaininfo &>/dev/null; then
        break
    fi
    sleep 5
done
...
while true; do
    # Get sync progress
    SYNC_INFO=$(${DIGIBYTE_DIR}/bin/digibyte-cli -conf=${DIGIBYTE_DIR}/digibyte.conf getblockchaininfo 2>/dev/null)
