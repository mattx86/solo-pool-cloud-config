...
if [ ! -f "${WALLET_DIR}/pool-wallet.privatekey" ]; then
    log "ERROR: ALEO wallet private key not found"
    log "Expected at: ${WALLET_DIR}/pool-wallet.privatekey"
    exit 1
fi

WALLET_ADDRESS=$(cat "${WALLET_DIR}/pool-wallet.address" 2>/dev/null)
