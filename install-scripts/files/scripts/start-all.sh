...
if [ "${ENABLE_BITCOIN_POOL}" = "true" ]; then
    if [ "${DAEMON_MODE}" = "true" ]; then
        nohup ${BASE_DIR}/start-btc.sh >> "${LOG_DIR}/btc.log" 2>&1 &
    else
        ${BASE_DIR}/start-btc.sh &
    fi
    PIDS[btc]=$!
...
if [ "${ENABLE_BCH_POOL}" = "true" ]; then
    if [ "${DAEMON_MODE}" = "true" ]; then
        nohup ${BASE_DIR}/start-bch.sh >> "${LOG_DIR}/bch.log" 2>&1 &
    else
        ${BASE_DIR}/start-bch.sh &
    fi
    PIDS[bch]=$!
...
if [ "${ENABLE_DGB_POOL}" = "true" ]; then
    if [ "${DAEMON_MODE}" = "true" ]; then
        nohup ${BASE_DIR}/start-dgb.sh >> "${LOG_DIR}/dgb.log" 2>&1 &
    else
        ${BASE_DIR}/start-dgb.sh &
    fi
    PIDS[dgb]=$!
...
if [ "${ENABLE_MONERO_POOL}" = "true" ]; then
    if [ "${DAEMON_MODE}" = "true" ]; then
        nohup ${BASE_DIR}/start-xmr.sh >> "${LOG_DIR}/xmr.log" 2>&1 &
    else
        ${BASE_DIR}/start-xmr.sh &
    fi
    PIDS[xmr]=$!
...
if [ "${ENABLE_TARI_POOL}" = "true" ] && [ "${MONERO_TARI_MODE}" != "monero_only" ]; then
    if [ "${DAEMON_MODE}" = "true" ]; then
        nohup ${BASE_DIR}/start-xtm.sh >> "${LOG_DIR}/xtm.log" 2>&1 &
    else
        ${BASE_DIR}/start-xtm.sh &
    fi
    PIDS[xtm]=$!
...
if [ "${ENABLE_ALEO_POOL}" = "true" ]; then
    if [ "${DAEMON_MODE}" = "true" ]; then
        nohup ${BASE_DIR}/start-aleo.sh >> "${LOG_DIR}/aleo.log" 2>&1 &
    else
        ${BASE_DIR}/start-aleo.sh &
    fi
    PIDS[aleo]=$!
...
echo ""
echo "Check status: ${BASE_DIR}/status.sh"
